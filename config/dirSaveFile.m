...
if ~isdir(sprintf('data/%s/fmap/',const.sjct))
    mkdir(sprintf('data/%s/fmap/',const.sjct))
end

% Define directory
const.dat_output_file   =   sprintf('data/%s/fmap/%s_task-%s%s_run-%i',const.sjct,const.sjct,const.cond1_txt,const.cond2_txt,const.cond_run_num(const.runNum));
