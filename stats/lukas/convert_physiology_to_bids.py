...
class PhilipsPhysioLog:
    ...
    def plot_alignment(self, win=4000, out_dir=None):
        ...
        amp = np.max([self.align_grad.min() * .25, 1])
...
if __name__ == '__main__':
    import nibabel as nib
    from glob import glob
    logs = glob('../sub-13/ses-mapper1/func/sub-13*_physio.log')
    for log in logs:
        nii = log.replace('_physio.log', '_bold.nii.gz')
        vols = nib.load(nii).shape[-1]
        l = PhilipsPhysioLog(f=log, tr=1.32, n_dyns=vols, sf=500).load().align(trigger_method='vol_triggers') 
        #l.plot_alignment(out_dir='../derivatives/physio_out')
        l.to_bids()
